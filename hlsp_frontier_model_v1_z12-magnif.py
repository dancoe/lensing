...
def savemagnif(inkappa, ingamma1, ingamma2, zl, zsin, outfile, zsout):
    ...
    gamma1 = pyfits.open(ingamma1)[0].data
    gamma2 = pyfits.open(ingamma2)[0].data
...
if __name__ == '__main__':
    if not len(sys.argv) > 1:
        print
        print 'USAGE: python hlsp_frontier_model_v1_z-magnif.py inkappa.fits ingamma1.fits ingamma2.fits lensredshift inredshift outmagnif.fits outredshift'
        print 'EXAMPLE: python hlsp_frontier_model_v1_z-magnif.py hlsp_frontier_model_abell2744_cats_v1_kappa.fits hlsp_frontier_model_abell2744_cats_v1_gamma1.fits hlsp_frontier_model_abell2744_cats_v1_gamma2.fits 0.308 inf magnif_z11.fits 11'
        print 'Requires scipy and pyfits'
        print
        quit()

    print len(sys.argv)
    inkappa, ingamma1, ingamma2, zl, zsin, outfile, zsout = sys.argv[1:]
    zl = float(zl)     # Lens redshift
    zsin = float(zsin)   # Source redshift images are calibrated to (input)
    zsout = float(zsout)  # Source redshift you are interested in (output)
    savemagnif(inkappa, ingamma1, ingamma2, zl, zsin, outfile, zsout)
